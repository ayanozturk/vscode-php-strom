...
func TestDiagnosticsProviderAcceptsDirectPHPDocListConstructorArguments(t *testing.T) {
	source := `<?php
...
final class EntryPage
{
    ...
    public function __construct(public array $entries, public int $page) {}
}

$empty = new EntryPage([], 1);
$populated = new EntryPage(entries: [new Entry()], page: 2);`
	diagnostics := (&DiagnosticsProvider{}).Analyse("file:///list-constructor.php", source)
	if hasDiagnosticCode(diagnostics, "A.ARG.TYPE") {
		t.Fatalf("expected PHPDoc list constructor parameters to accept array literals, got %#v", diagnostics)
	}
}
